...
//
// CPU (free-план 10ms/вызов): главный пожиратель — это .toLowerCase() всей страницы
// (копия большой строки). Поэтому ищем регэкспом с флагом i прямо по html — без копии
// и с остановкой на первом совпадении. Страницу НЕ режем: баннер продажи на Fragment
// рендерится ниже шапки.
...
export async function checkTelegram(username: string): Promise<"free" | "taken"> {
  try {
    ...
    // .includes() дёшево и останавливается на первом совпадении. Маркеры — только у реального аккаунта.
    const occupied =
...
export async function checkFragment(username: string): Promise<"free" | "sale" | "taken"> {
  try {
    ...
    const html = await res.text();
    // Флаг i вместо toLowerCase() — нет копии всей страницы, .test() рвётся на 1-м совпадении.
